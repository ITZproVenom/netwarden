...
import { useControlAudit, useDisconnectDevice, useRestoreControl, useStartContinuousControl, useStopContinuousControl } from "@/features/control/control.queries"
...
function ControlSection({ device }: { device: Device }) {
  ...
  const recent = audit.filter(event => event.targets.some(target => target.toLowerCase().includes(device.mac.toLowerCase()))).slice(0, 3)
  ...
  const controlled = device.controlState === "active" || device.controlState === "continuous" || device.controlState === "restoring" || device.controlState === "failed"
  const label = device.controlState === "continuous" ? "Continuous" : device.controlState === "restoring" ? "Restoring" : device.controlState === "failed" ? "Recovery failed" : device.controlState === "active" ? "Disconnected" : eligible ? "Available" : device.role !== "Device" ? "Protected" : "Offline"
  return <section className="space-y-3 py-6"><div className="flex items-center justify-between"><h4 className="text-xs font-semibold">Device control</h4><Badge variant={controlled ? "destructive" : eligible ? "outline" : "secondary"}>{(pending || device.controlState === "restoring") && <LoaderCircle className="animate-spin" />}{label}</Badge></div><div className="flex flex-wrap gap-2">{controlled ? <><ControlConfirmation title="Restore device" description={`Restore normal network access for ${device.name}?`} disabled={recovering} onConfirm={() => restore.mutate({ ip: device.ip, mac: device.mac })}><RotateCcw />Restore</ControlConfirmation>{device.controlState === "continuous" && <ControlConfirmation title="Stop continuous control" description={`Stop continuous control and restore ${device.name}?`} disabled={recovering} onConfirm={() => stop.mutate({ ip: device.ip, mac: device.mac })}><RadioTower />Stop continuous</ControlConfirmation>}</> : <><ControlConfirmation title="Disconnect device" description={`Disconnect ${device.name} from the network?`} disabled={!eligible || pending} onConfirm={() => disconnect.mutate({ ip: device.ip, mac: device.mac })}><Ban />Disconnect</ControlConfirmation><ControlConfirmation title="Start continuous control" description={`Continuously prevent ${device.name} from reconnecting until explicitly restored?`} disabled={!eligible || pending} onConfirm={() => continuous.mutate({ ip: device.ip, mac: device.mac })}><RadioTower />Continuous</ControlConfirmation></>}</div>{recent.length > 0 && <div className="rounded-lg border bg-background/30 p-3"><p className="mb-2 text-[10px] font-semibold tracking-wide text-muted-foreground uppercase">Recent control activity</p>{recent.map((event, index) => <div key={`${event.at}-${index}`} className="flex justify-between gap-3 py-1 text-[10px]"><span>{event.operation.replaceAll("_", " ")}</span><span className="text-muted-foreground">{event.outcome.replaceAll("_", " ")}</span></div>)}</div>}</section>
}

function ControlConfirmation({ title, description, disabled, onConfirm, children }: { title: string; description: string; disabled: boolean; onConfirm: () => void; children: React.ReactNode }) { return <AlertDialog><AlertDialogTrigger asChild><Button variant="outline" size="sm" disabled={disabled}>{children}</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{title}?</AlertDialogTitle><AlertDialogDescription>{description}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={onConfirm}>Confirm</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog> }

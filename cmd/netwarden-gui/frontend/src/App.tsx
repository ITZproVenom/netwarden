...
export default function App() {
  ...
  const [view, setView] = useState<'devices' | 'integrity' | 'settings'>('devices')
  const [gatewayMAC, setGatewayMAC] = useState(''), [savedGatewayMAC, setSavedGatewayMAC] = useState('')
  ...
  useEffect(() => {
    api().Bootstrap().then((data) => { setAdapters(data.interfaces); setSelected(data.selectedInterface || data.interfaces[0]?.name || ''); setGatewayMAC(data.gatewayMAC || ''); setSavedGatewayMAC(data.gatewayMAC || '') }).catch(showError).finally(() => setLoading(false))
  ...
  async function saveGateway() { setError(''); try { const saved = await api().SetGatewayMAC(gatewayMAC.trim()); setGatewayMAC(saved); setSavedGatewayMAC(saved) } catch (reason) { showError(reason) } }

  return <main>
    <header><div className="brand"><span className="mark">N</span><div><h1>NetWarden</h1><p>Local network visibility</p></div></div><nav><button className={view === 'devices' ? 'selected' : ''} onClick={() => setView('devices')}>Devices</button><button className={view === 'integrity' ? 'selected' : ''} onClick={() => setView('integrity')}>Integrity {status?.ConflictCount ? <b>{status.ConflictCount}</b> : null}</button><button className={view === 'settings' ? 'selected' : ''} onClick={() => setView('settings')}>Settings</button></nav><span className={`state ${running ? 'active' : ''}`}><i />{running ? 'Monitoring' : 'Stopped'}</span></header>
    ...
    </section> : view === 'integrity' ? <section className="panel"><div className="panelHead"><div><h3>Gateway integrity</h3><p>Claims that differed from the trusted gateway identity</p></div><span className={`integrityState ${conflicts.some(c => c.active) ? 'warning' : ''}`}>{conflicts.some(c => c.active) ? 'Active warning' : 'No active warnings'}</span></div>
    ...
    </section> : <section className="panel settingsPanel"><div className="panelHead"><div><h3>Settings</h3><p>Configure how NetWarden identifies this network</p></div></div><div className="settingsBody"><div className="settingCopy"><h4>Trusted gateway identity</h4><p>Pin the hardware address of your router. NetWarden will report any gateway claim from a different MAC address. Leave this empty to learn the gateway identity when monitoring starts.</p>{running && <span className="settingNotice">Stop monitoring to change this setting.</span>}</div><div className="settingControl"><label htmlFor="gateway-mac">Gateway MAC address</label><input id="gateway-mac" spellCheck={false} placeholder="00:11:22:33:44:55" value={gatewayMAC} disabled={running} onChange={e => setGatewayMAC(e.target.value)} /><small>{savedGatewayMAC ? `Pinned to ${savedGatewayMAC}` : 'Automatically learned'}</small><div><button disabled={running || gatewayMAC.trim().toLowerCase() === savedGatewayMAC.toLowerCase()} className="primary" onClick={saveGateway}>Save baseline</button>{savedGatewayMAC && <button disabled={running} onClick={async () => { setError(''); try { await api().SetGatewayMAC(''); setGatewayMAC(''); setSavedGatewayMAC('') } catch (reason) { showError(reason) } }}>Use automatic</button>}</div></div></div></section>}

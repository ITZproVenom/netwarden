...
func NewBandwidthService(controller BandwidthController, devices bandwidthDeviceSource, blocked func(net.HardwareAddr) bool) *BandwidthService {
	return &BandwidthService{controller: controller, devices: devices, blocked: blocked, active: make(map[string]BandwidthTarget)}
}
...
func (s *BandwidthService) remove(ctx context.Context, mac net.HardwareAddr) error {
	...
	if err := s.controller.RemoveBandwidthLimit(ctx, target.IP, target.MAC); err != nil {
		return fmt.Errorf("remove bandwidth limit: %w", err)
	}
	s.mu.Lock()
	delete(s.active, key)
	s.mu.Unlock()
